...
strictStr = Annotated[str, StringConstraints(
...
def update_user_account(user_id: int, input_dict : AccountUpdateModal):
    controller: UpdateUserAccountController = UpdateUserAccountController()
    try:
        input_dict = input_dict.model_dump(exclude_unset=True)
        controller.updateUserAccount(user_id, input_dict)
    except Exception:
        raise HTTPException(
            status_code=404, detail="Failed to update Accounts")
    return True
...
def suspend_user_account(user_id: int) -> bool:
    ...
    try:
        ...
        print(e)
        raise HTTPException(
            status_code=404, detail="Failed to suspend account")
# Search (backend-way)
...
def create_user_profile(input: ProfileModal):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to create Profile")
...
def view_user_profiles_list():
    ...
    try:
        ...
        print(e.msg)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No Profiles found")
...
def suspend_user_profile(profile_id: int) -> bool:
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="Failed to suspend user profile.")
