    ...
    // All search/filter operations are sent to the DB as query params
    function buildQS() {
    ...
    // ── DB call: GET /donee/donations[?keyword=&category_id=&date_from=&date_to=]
    // Returns all if no params, filters at SQL JOIN level if params present
    async function dbSearchDonations() {
        const qs = buildQS();
    ...
    // ── Load summary stats (one DB call, no filter) ──
    async function loadSummary() {
        try {
            ...
            const seen = {};
            all.forEach(d=>{ if(d.category_id&&d.category_name) seen[d.category_id]=d.category_name; });
    ...
    function renderTable(donations) {
        ...
        tbody.innerHTML = '';
        const filtered_total = donations.reduce((s,d)=>s+parseFloat(d.amount||0),0);
        document.getElementById('result-info').innerHTML =
            `Showing <span>${donations.length}</span> donation${donations.length!==1?'s':''}` +
            (donations.length?` · Total: <span>${fmt(filtered_total)}</span>`:'');

        if(!donations.length){
            tbody.innerHTML=`<tr><td colspan="6"><div class="dh-empty"><div class="dh-empty-icon"><i class="bi bi-clock-history"></i></div><div class="dh-empty-title">No donations found</div><div class="dh-empty-sub">Try adjusting your search or date range.</div></div></td></tr>`;
            return;
        }
        ...
        donations.forEach((d,i)=>{
            ...
            tr.innerHTML=`
                ...
                <td>${d.category_name?`<span class="dh-cat"><i class="bi bi-tag-fill"></i>${esc(d.category_name)}</span>`:'<span style="color:#ddd">—</span>'}</td>
    ...
    // ── Load table — always a DB call ──
    async function loadTable() {
        try {
            const donations = await dbSearchDonations();
            renderTable(donations);
        } catch(_){
            document.getElementById('dh-tbody').innerHTML=`<tr><td colspan="6"><div class="dh-empty"><div class="dh-empty-icon"><i class="bi bi-exclamation-circle"></i></div><div class="dh-empty-title">Could not load donations.</div></div></td></tr>`;
        }
    }
        ...
        document.getElementById('don-detail').innerHTML=`
            ...
            <div class="don-detail-grid">
                ...
                <div class="don-detail-item">
                    ...
                    <div class="don-detail-val" style="color:${fraActive?'#16a34a':'#c0392b'}">${fraActive?'● Active':'● Ended'}</div>
                </div>
                <div class="don-detail-item">
            ...
            ${d.description?`<div><div style="font-size:.72rem;font-weight:700;color:#aaa;text-transform:uppercase;letter-spacing:.4px;margin-bottom:5px;">About this campaign</div><p style="font-size:.85rem;color:#666;line-height:1.6;margin:0;">${esc(d.description)}</p></div>`:''}`;
    ...
    // ── Filter events ──
    let debounce;
    document.getElementById('f-keyword').addEventListener('input', ()=>{ clearTimeout(debounce); debounce=setTimeout(loadTable,350); });
    ...
    // ── Init ──
    (async () => {
        await loadSummary();  // DB call — totals + populate categories dropdown
        await loadTable();    // DB call — all donations (no filter initially)
    })();